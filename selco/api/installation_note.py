...
def get_installation_note():
	...
	for row in data:
		...
		parent_dict['packed_items'] = frappe.db.get_values("Packed Item",{'parenttype':'Installation Note','parent':row.name},child_fields, order_by="idx",as_dict=True)
		data_list.append(parent_dict)
...
def update_installation_note():
	...
	if frappe.request.data:
		...
		try:
			doc.selco_custmer_date = doc.selco_cse_date
			doc.save()
